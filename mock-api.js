...
async function fetchSiteEnergyHistory(siteKey, period) {
  ...
  switch (period) {
    case 'today':
      intervals = 24; stepMs = 3600000;
      labelFn = (d) => `${d.getHours()}:00`;
      break;
    case '5days':
      intervals = 5; stepMs = 86400000;
      labelFn = (d) => d.toLocaleDateString('en', { weekday: 'short', day: 'numeric' });
      break;
    case 'month':
      intervals = 30; stepMs = 86400000;
      labelFn = (d) => d.toLocaleDateString('en', { day: 'numeric', month: 'short' });
      break;
    case '5months':
      intervals = 5; stepMs = 30 * 86400000;
      labelFn = (d) => d.toLocaleDateString('en', { month: 'short', year: '2-digit' });
      break;
    case 'year':
      intervals = 12; stepMs = 30 * 86400000;
      labelFn = (d) => d.toLocaleDateString('en', { month: 'short', year: '2-digit' });
      break;
    default:
      intervals = 6; stepMs = 30 * 86400000;
      labelFn = (d) => d.toLocaleDateString('en', { month: 'short', year: '2-digit' });
  }

  const scaleMap = {today: 1/24, '5days': 1/30, month: 1/30, '5months': 1, year: 1};
...
async function fetchAllSwissSites(cfg) {
  const results = [];
  const elecPrice = (cfg && cfg.energyPrice) ? cfg.energyPrice : 0.18;
  for (const [site, info] of Object.entries(SWISS_SITES)) {
    ...
    // FIX: Avoidable MWh multiplied by price (1 MWh = 1,000 kWh)
    const avoidableMwh = Math.max(0, mwh - base);
    const savingsChf = +(avoidableMwh * elecPrice * 1000).toFixed(0);
